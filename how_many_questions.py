...
class ChooseQuestion:
    ...
    def __init__(self):
        ...
        choose_instructions_txt = "In each question there will be 4 stadiums " \
                                  "to choose from, " \
        ...
        for item in range(0, 3):
            self.question_button = Button(self.how_many_frame,
                                        fg=button_fg, bg=btn_stadium_value[item][0],
                                        text="{} Question".format(btn_stadium_value[item][1]),
                                        font=button_font, width=10,
                                        command=lambda i=item: self.to_play(btn_stadium_value[i][1])
                                        )
